...
/* STS Minimum Threshold (STS_MNTH) needs to be adjusted with changing STS
length. To adjust the STS_MNTH following formula can be used: STS_MNTH =
SQRT(X/Y)*default_STS_MNTH default_STS_MNTH is 0x10 X is the length of the STS
in units of 8 (i.e. 8 for 64 length, 16 for 128 length etc.) Y is either 8 or
16, 8 when no PDOA or PDOA mode 1 and 16 for PDOA mode 3

The API does not use the formula and the STS_MNTH value is derived from
approximation formula as given by get_sts_mnth() function. The API here supports
STS lengths as listed in: dwt_sts_lengths_e enum, which are: 32, 64, 128, 256,
512, 1024, 2048 The enum value is used as the index into sts_length_factors
array. The array has values which are generated by: val =
SQRT(stsLength/16)*2048
*/
const uint16_t sts_length_factors[STS_LEN_SUPPORTED] = {1024, 1448, 2048, 2896,
                                                        4096, 5793, 8192};

static uint16_t get_sts_mnth(uint16_t sts, uint8_t default_threshold,
                             uint8_t shift_val);
...
#define CIA_MANUALLOWERBOUND_TH_64                                             \
  (0x10) // cia lower bound threshold values for 64 MHz PRF
...
static void
_dwt_otpprogword32(uint32_t data,
                   uint16_t address); // Program the non-volatile memory
...
typedef struct {
  ...
  uint8_t
      vBatP; // IC V bat read during production and stored in OTP (Vmeas @ 3V3)
  uint8_t
      tempP; // IC temp read during production and stored in OTP (Tmeas @ 23C)
  uint8_t longFrames;  // Flag in non-standard long frame mode
  uint8_t otprev;      // OTP revision number (read during initialisation)
  uint8_t init_xtrim;  // initial XTAL trim value read from OTP (or defaulted to
                       // mid-range if OTP not programmed)
  uint8_t dblbuffon;   // Double RX buffer mode and DB status flag
  uint16_t sleep_mode; // Used for automatic reloading of LDO tune and microcode
                       // at wake-up
...
static dwt_local_data_t
    DW3000local[DWT_NUM_DW_DEV]; // Local device data, can be an array to
                                 // support multiple DW3000 testing
                                 // applications/platforms
static dwt_local_data_t *pdw3000local =
    &DW3000local[0]; // Local data structure pointer
static uint8_t crcTable[256];

/*!
 * ------------------------------------------------------------------------------------------------------------------
 * @brief This function returns the version of the API as defined by
 * DW3000_DRIVER_VERSION
 *
 * input parameters
 *
 * output parameters
 ...
 */
int32_t dwt_apiversion(void) { return DW3000_DRIVER_VERSION; }
...
static void dwt_xfer3000(
    const uint32_t regFileID, // 0x0, 0x04-0x7F ; 0x10000, 0x10004,
                              // 0x10008-0x1007F; 0x20000 etc
  ...
  assert(mode == DW3000_SPI_WR_BIT || mode == DW3000_SPI_RD_BIT ||
         mode == DW3000_SPI_AND_OR_8 || mode == DW3000_SPI_AND_OR_16 ||
  ...
  header[0] = (uint8_t)((mode | addr) >>
                        8); //  & 0xFF; //bit7 + addr[4:0] + sub_addr[6:6]
  header[1] =
      (uint8_t)(addr |
                (mode & 0x03)); // & 0xFF; //EAM: subaddr[5:0]+ R/W/AND_OR

  if (/*reg_offset == 0 && */ length ==
      0) { /* Fast Access Commands (FAC)
            * only write operation is possible for this mode
            * bit_7=one is W operation, bit_6=zero: FastAccess command,
            * bit_[5..1] addr, bits_0=one: MODE of FastAccess
            */
    assert(mode == DW3000_SPI_WR_BIT);

    header[0] =
        (uint8_t)((DW3000_SPI_WR_BIT >> 8) | (regFileID << 1) | DW3000_SPI_FAC);
    cnt = 1;
  } else if (reg_offset == 0 /*&& length > 0*/ &&
             (mode == DW3000_SPI_WR_BIT ||
              mode ==
                  DW3000_SPI_RD_BIT)) { /* Fast Access Commands with Read/Write
                                         * support (FACRW) bit_7 is R/W
                                         * operation, bit_6=zero: FastAccess
                                         * command, bit_[5..1] addr,
                                         * bits_0=zero: MODE of FastAccess
                                         */
  ...
  case DW3000_SPI_RD_BIT: {
    ...
    // also don't do for SPICRC_CFG_ID register itself to prevent infinite
    // recursion
    if ((pdw3000local->spicrc == DWT_SPI_CRC_MODE_WRRD) &&
      ...
      // potential problem in callback if it will try to read/write SPI with CRC
      // again.
      if (crc8 != dwcrc8) {
 ...
 * pins can be used for this. wakeup_device_with_io() which is external to this
 * file and is platform dependant and it should be modified to toggle the
 * correct pin depending on the HW/MCU connections with DW3000.
 *
 * @param None
 *
 * output parameters
 ...
 * no return value
 */
void dwt_wakeup_ic(void) { wakeup_device_with_io(); }
...
/*!
 * ------------------------------------------------------------------------------------------------------------------
 * @brief  this function is used to read 32-bit value from the DW3000 device
 * registers
 *
 * input parameters:
...
/*!
 * ------------------------------------------------------------------------------------------------------------------
 * @brief  this function is used to read 16-bit value from the DW3000 device
 * registers
 *
 * input parameters:
 * @param regFileID - ID of register file or buffer being accessed
...
/*!
 * ------------------------------------------------------------------------------------------------------------------
 * @brief  this function is used to read an 8-bit value from the DW3000 device
 * registers
 *
 * input parameters:
 * @param regFileID - ID of register file or buffer being accessed
...
/*!
 * ------------------------------------------------------------------------------------------------------------------
 * @brief  this function is used to write 32-bit value to the DW3000 device
 * registers
 *
 * input parameters:
 * @param regFileID - ID of register file or buffer being accessed
...
/*!
 * ------------------------------------------------------------------------------------------------------------------
 * @brief  this function is used to write 16-bit value to the DW3000 device
 * registers
 *
 * input parameters:
 * @param regFileID - ID of register file or buffer being accessed
...
/*!
 * ------------------------------------------------------------------------------------------------------------------
 * @brief  this function is used to write an 8-bit value to the DW3000 device
 * registers
 *
 * input parameters:
 * @param regFileID - ID of register file or buffer being accessed
...
/*!
 * ------------------------------------------------------------------------------------------------------------------
 * @brief  this function is used to modify a 32-bit value to the DW3000 device
 * registers
 *
 * input parameters:
 * @param regFileID :   ID of register file or buffer being accessed
 ...
 */
void dwt_modify32bitoffsetreg(const int regFileID, const int regOffset,
                              const uint32_t _and, const uint32_t _or) {
  uint8_t buf[8];
...
/*!
 * ------------------------------------------------------------------------------------------------------------------
 * @brief  this function is used to modify a 16-bit value to the DW3000 device
 * registers
 *
 * input parameters:
 * @param regFileID :   ID of register file or buffer being accessed
 ...
 * @output          :   no return value
 */
void dwt_modify16bitoffsetreg(const int regFileID, const int regOffset,
                              const uint16_t _and, const uint16_t _or) {
  uint8_t buf[4];
  buf[0] = (uint8_t)_and;        //       &0xFF;
  buf[1] = (uint8_t)(_and >> 8); //  &0xFF;
...
/*!
 * ------------------------------------------------------------------------------------------------------------------
 * @brief  this function is used to modify a 8-bit value to the DW3000 device
 * registers
 *
 * input parameters:
 * @param regFileID :   ID of register file or buffer being accessed
 ...
 * @output          :   no return value
 */
void dwt_modify8bitoffsetreg(const int regFileID, const int regOffset,
...
static void _dwt_crc8init(void) {
  uint8_t remainder;
  int dividend;
 ...
 * polynomial (i.e. P(x) = x^8+ x^2+ x^1+ x^0) this function has been optimized
 * to use crcTable[] and calculate the CRC on byte by byte basis.
 *
 * input parameters:
 * @param byteArray         - data to calculate CRC for
 * @param len               - length of byteArray
 * @param crcRemainderInit  - the remainder is the crc, also it is initially set
 * to the initialisation value for CRC calculation
 *
 * output parameters
 *
 ...
 * performed in DW3000 on each SPI write last byte of the SPI write transaction
 * needs to be the 8-bit CRC, if it does not match the one calculated by DW3000
 * SPI CRC ERROR event will be set in the status register (SYS_STATUS_SPICRC)
 *
 * @param spireaderr_cb - this needs to contain the callback function pointer
 * which will be called when SPI read error is detected (when the DW3000
 * generated CRC does not match the one calculated by  dwt_generatecrc8
...
void dwt_enablespicrccheck(dwt_spi_crc_mode_e crc_mode,
  ...
  if (crc_mode != DWT_SPI_CRC_MODE_NO) // enable CRC check in DW3000
  {
    ...
    if (crc_mode == DWT_SPI_CRC_MODE_WRRD) // enable CRC generation on SPI read
                                           // transaction which the DW3000 will
                                           // store in SPICRC_CFG_ID register
    {
...
static void _dwt_prog_ldo_and_bias_tune(void) {
...
/*!
 * ------------------------------------------------------------------------------------------------------------------
 * @brief This function is a static function used to 'kick' the desired
 * operating parameter set (OPS) table upon wakeup from sleep. It will load the
 * required OPS table configuration based upon what OPS table was set to be used
 * in dwt_configure().
 *
 * input parameters
 *
...
static void _dwt_kick_ops_table_on_wakeup(void) {
  ...
  /* Using the mask of all available OPS table options, check for OPS table
   * options in the sleep mode mask */
  switch (pdw3000local->sleep_mode &
          (DWT_ALT_OPS | DWT_SEL_OPS0 | DWT_SEL_OPS1 | DWT_SEL_OPS2 |
           DWT_SEL_OPS3)) {
  /* If preamble length >= 256 and set by dwt_configure(), the OPS table should
   * be kicked off like so upon wakeup. */
  case (DWT_ALT_OPS | DWT_SEL_OPS0):
    ...
    break;
  /* If SCP mode is enabled by dwt_configure(), the OPS table should be kicked
   * off like so upon wakeup. */
...
/*!
 * ------------------------------------------------------------------------------------------------------------------
 * @brief This function is a static function used to 'kick' the DGC upon wakeup
 * from sleep. It will load the required DGC configuration from OTP based upon
 * what channel was set to be used in dwt_configure().
 *
 * input parameters
 ...
 * initialises as necessary any static data items belonging to this low-level
 * driver.
 *
 * NOTES:
 * 1.it also reads and applies LDO and BIAS tune and crystal trim values from
 * OTP memory 2.it is assumed this function is called after a reset or on power
 * up of the DW3000
 *
 * input parameters
...
int dwt_initialise(int mode) {
  ...
  pdw3000local->dblbuffon =
      DBL_BUFF_OFF; // Double buffer mode off by default / clear the flag
  pdw3000local->sleep_mode = DWT_RUNSAR; // Configure RUN_SAR on wake by default
                                         // as it is needed when running PGF_CAL
  pdw3000local->spicrc = (dwt_spi_crc_mode_e)0;
  ...
  pdw3000local->bias_tune =
      (_dwt_otpread(BIAS_TUNE_ADDRESS) >> 16) & BIAS_CTRL_BIAS_MASK;
  ...
  if (pdw3000local->tempP ==
      0) // if the reference temperature has not been programmed in OTP (early
         // eng samples) set to default value
  {
    pdw3000local->tempP = 0x85; //@temp of 20 deg
  }

  if (pdw3000local->vBatP ==
      0) // if the reference voltage has not been programmed in OTP (early eng
 ...
 * DWT_DW_INIT (0) to put DW3000 into INIT_RC state; DWT_DW_IDLE_RC (2) to put
 * DW3000 into IDLE_RC state.
 *
...
void dwt_setdwstate(int state) {
  if (state == DWT_DW_IDLE) // Set the auto INIT2IDLE bit so that DW3000 enters
                            // IDLE mode before switching clocks to system_PLL
  // NOTE: PLL should be configured prior to this, and the device should be in
  // IDLE_RC (if the PLL does not lock device will remain in IDLE_RC)
  {
    // switch clock to auto - if coming here from INIT_RC the clock will be
    // FOSC/4, need to switch to auto prior to setting auto INIT2IDLE bit
    dwt_force_clocks(FORCE_CLK_AUTO);
    dwt_or8bitoffsetreg(SEQ_CTRL_ID, 0x01, SEQ_CTRL_AINIT2IDLE_BIT_MASK >> 8);
  } else if (state == DWT_DW_IDLE_RC) // Change state to IDLE_RC and clear auto
                                      // INIT2IDLE bit
  {
  ...
  // NOTE: the SPI rate needs to be <= 7MHz as device is switching to INIT_RC
  // state
  {
 ...
 */
uint8_t dwt_otprevision(void) { return pdw3000local->otprev; }

/*!
 * ------------------------------------------------------------------------------------------------------------------
 * @brief This function enables/disables the fine grain TX sequencing (this is
 * enabled by default in the DW3000).
 *
 * input parameters
 ...
 * dependent, consult the DW3000 User Manual. This can also be used for debug as
 * enabling TX and RX GPIOs is quite handy to monitor DW3000's activity.
 *
 * NOTE: Enabling PA functionality requires that fine grain TX sequencing is
 * deactivated. This can be done using dwt_setfinegraintxseq().
 *
 * input parameters
...
void dwt_setlnapamode(int lna_pa) {
  ...
  gpio_mode &=
      (~(GPIO_MODE_MSGP0_MODE_BIT_MASK | GPIO_MODE_MSGP1_MODE_BIT_MASK |
         GPIO_MODE_MSGP4_MODE_BIT_MASK | GPIO_MODE_MSGP5_MODE_BIT_MASK |
         GPIO_MODE_MSGP6_MODE_BIT_MASK)); // clear GPIO 4, 5, 6, configuration
...
/*!
 * ------------------------------------------------------------------------------------------------------------------
 * @brief This is used to return the read V measured @ 3.3 V value recorded in
 ...
 */
uint8_t dwt_geticrefvolt(void) { return pdw3000local->vBatP; }

/*!
 * ------------------------------------------------------------------------------------------------------------------
 * @brief This is used to return the read T measured @ 23 C value recorded in
 ...
 */
uint8_t dwt_geticreftemp(void) { return pdw3000local->tempP; }
 ...
 */
uint32_t dwt_getpartid(void) { return pdw3000local->partID; }
 ...
 */
uint32_t dwt_getlotid(void) { return pdw3000local->lotID; }

/*!
 * ------------------------------------------------------------------------------------------------------------------
 * @brief This is used to return the read device type and revision information
 * of the DW3000 device (MP part is 0xDECA0300)
 *
 * input parameters
 *
 * output parameters
 ...
 */
uint32_t dwt_readdevid(void) { return dwt_read32bitoffsetreg(DEV_ID_ID, 0); }

/*!
 * ------------------------------------------------------------------------------------------------------------------
 * @brief This function provides the API for the configuration of the TX
 * spectrum including the power and pulse generator delay. The input is a
 * pointer to the data structure of type dwt_txconfig_t that holds all the
 * configurable items.
 *
 * input parameters
 * @param config    -   pointer to the txrf configuration structure, which
 * contains the tx rf config data If config->PGcount == 0 the PGdelay value will
 * be used, else the PG calibration will run
 *
 * output parameters
 *
 * no return value
...
static uint16_t get_sts_mnth(uint16_t cipher, uint8_t threshold,
                             uint8_t shift_val) {
  uint32_t value;
  uint16_t mod_val;
 ...
 * DEEPSLEEP/SLEEP state, to restore the configuration which has not been
 * automatically restored from AON
 *
 * input parameters
 *
 * return DWT_SUCCESS
...
void dwt_restoreconfig(void) {
  ...
  /*Restoring indirect access register B configuration as this is not preserved
   * when device is in DEEPSLEEP/SLEEP state. Indirect access register B is
   * configured to point to the "Double buffer diagnostic SET 2"*/
  ...
  // assume RX code is the same as TX (e.g. we will not RX on 16 MHz or SCP and
  // TX on 64 MHz)
 ...
 * DWT_STS_MODE_1 etc The dwt_configure should be called prior to this to
...
void dwt_configurestsmode(uint8_t stsMode) {
  ...
  // then set the relevant bits according to configuration of the PHR Mode, PHR
  // Rate, STS Protocol, SDC, PDOA Mode,
  dwt_modify32bitoffsetreg(
      SYS_CFG_ID, 0, ~(SYS_CFG_CP_SPC_BIT_MASK | SYS_CFG_CP_SDC_BIT_MASK),
 ...
 * structure of type dwt_config_t that holds all the configurable items. The
 * dwt_config_t structure shows which ones are supported
 *
 * input parameters
 ...
 * will be severely affected, the application should reset device and try again
 *
 */
int dwt_configure(dwt_config_t *config) {
  ...
  assert(((config->stsMode & DWT_STS_CONFIG_MASK) == DWT_STS_MODE_OFF) ||
         ...
         ((config->stsMode & DWT_STS_CONFIG_MASK) ==
          (DWT_STS_MODE_1 | DWT_STS_MODE_SDC)) ||
         ((config->stsMode & DWT_STS_CONFIG_MASK) ==
          (DWT_STS_MODE_2 | DWT_STS_MODE_SDC)) ||
         ((config->stsMode & DWT_STS_CONFIG_MASK) ==
          (DWT_STS_MODE_ND | DWT_STS_MODE_SDC)) ||
  ...
  pdw3000local->sleep_mode &=
      (~(DWT_ALT_OPS | DWT_SEL_OPS3)); // clear the sleep mode ALT_OPS bit
  pdw3000local->longFrames = config->phrMode;
  sts_len = GET_STS_REG_SET_VALUE((uint16_t)(config->stsLength));
  pdw3000local->ststhreshold =
      (int16_t)((((uint32_t)sts_len) * 8) * STSQUAL_THRESH_64);
  ...
  dwt_modify32bitoffsetreg(
      ...
      ((uint32_t)config->pdoaMode) << SYS_CFG_PDOA_MODE_BIT_OFFSET |
          ...
          (SYS_CFG_PHR_6M8_BIT_MASK &
           ((uint32_t)config->phrRate << SYS_CFG_PHR_6M8_BIT_OFFSET)) |
          mode);
  ...
  if (scp) {
    ...
    dwt_write32bitoffsetreg(
        IP_CONFIG_LO_ID, 0,
        IP_CONFIG_LO_SCP); // Set this if Ipatov analysis is used in SCP mode
  ...
  } else //
  {
    ...
    if (config->stsMode != DWT_STS_MODE_OFF) {
      ...
      if ((config->pdoaMode == DWT_PDOA_M1) ||
        ...
        // In PDOA mode 1, number of accumulated symbols is the whole length of
        // the STS
        sts_mnth =
            get_sts_mnth(sts_length_factors[(uint8_t)(config->stsLength)],
                         CIA_MANUALLOWERBOUND_TH_64, 3);
      } else {
        // In PDOA mode 3 number of accumulated symbols is half of the length of
        // STS symbols
        sts_mnth =
            get_sts_mnth(sts_length_factors[(uint8_t)(config->stsLength)],
  ...
  dwt_modify8bitoffsetreg(DTUNE0_ID, 0, (uint8_t)~DTUNE0_PRE_PAC_SYM_BIT_MASK,
                          config->rxPAC);

  dwt_write8bitoffsetreg(STS_CFG0_ID, 0,
                         sts_len - 1); /*Starts from 0 that is why -1*/

  if (config->txPreambLength == DWT_PLEN_72) {
    dwt_setplenfine(8); // value 8 sets fine preamble length to 72 symbols -
  ...
  dwt_modify32bitoffsetreg(
      TX_FCTRL_ID, 0, ~(TX_FCTRL_TXBR_BIT_MASK | TX_FCTRL_TXPSR_BIT_MASK),
  ...
  ///////////////////////
  // PGF
  error = dwt_pgf_cal(1); // if the RX calibration routine fails the device
                          // receiver performance will be severely affected, the
                          // application should reset and try again
 ...
 * to the tx buffer. Note: the size of tx buffer is 1024 bytes. The standard PHR
 * mode allows to transmit frames of up to 127 bytes (including 2 byte CRC) The
 * extended PHR mode allows to transmit frames of up to 1023 bytes (including 2
 * byte CRC) if > 127 is programmed, DWT_PHRMODE_EXT needs to be set in the
 * phrMode configuration see dwt_configure function
 * @param txDataBytes    - Pointer to the user뭩 buffer containing the data to
 * send.
 * @param txBufferOffset - This specifies an offset in the DW IC뭩 TX Buffer at
...
int dwt_writetxdata(uint16_t txDataLength, uint8_t *txDataBytes,
  ...
  if ((txBufferOffset + txDataLength) < TX_BUFFER_MAX_LEN) {
    if (txBufferOffset <= REG_DIRECT_OFFSET_MAX_LEN) {
      ...
      dwt_writetodevice(TX_BUFFER_ID, txBufferOffset, txDataLength,
                        txDataBytes);
    } else {
      /* Program the indirect offset register A for specified offset to TX
       * buffer */
 ...
 * CRC) - max is 1023 NOTE: standard PHR mode allows up to 127 bytes if > 127 is
 * programmed, DWT_PHRMODE_EXT needs to be set in the phrMode configuration see
...
void dwt_writetxfctrl(uint16_t txFrameLength, uint16_t txBufferOffset,
  ...
  // DW3000/3700 - if offset is > 127, 128 needs to be added before data is
  // written, this will be subtracted internally prior to writing the data
  if (txBufferOffset <= 127) {
    // Write the frame length to the TX frame control register
    reg32 = txFrameLength |
            ((uint32_t)(txBufferOffset) << TX_FCTRL_TXB_OFFSET_BIT_OFFSET) |
    ...
    dwt_modify32bitoffsetreg(TX_FCTRL_ID, 0,
                             ~(TX_FCTRL_TXB_OFFSET_BIT_MASK |
                               TX_FCTRL_TR_BIT_MASK | TX_FCTRL_TXFLEN_BIT_MASK),
                             reg32);
    reg32 = dwt_read8bitoffsetreg(SAR_CTRL_ID,
                                  0); // DW3000/3700 - need to read this to load
                                      // the correct TX buffer offset value
  }
 ...
 * frame premable length can be configured in steps of 8, from 16 to 2048
 * symbols. If a non-zero value is configured, then the TXPSR_PE setting is
 * ignored.
 *
 * input parameters:
 * @param preambleLength - sets the length of the preamble, value of 0 disables
 * this setting and the length of the frame will be dependent on the TXPSR_PE
 * setting as configured by dwt_configure function
 *
 * output parameters
 *
 * no return value
...
void dwt_readrxdata(uint8_t *buffer, uint16_t length, uint16_t rxBufferOffset) {
  ...
  if (pdw3000local->dblbuffon ==
      DBL_BUFF_ACCESS_BUFFER_1) // if the flag is 0x3 we are reading from
                                // RX_BUFFER_1
  {
  ...
  if ((rxBufferOffset + length) <= RX_BUFFER_MAX_LEN) {
    if (rxBufferOffset <= REG_DIRECT_OFFSET_MAX_LEN) {
      ...
      /* Program the indirect offset registers B for specified offset to RX
       * buffer */
 ...
 * an offset location give by offset parameter for 18 bit complex samples, each
 * sample is 6 bytes (3 real and 3 imaginary)
 *
 *
 * NOTE: Because of an internal memory access delay when reading the accumulator
 * the first octet output is a dummy octet that should be discarded. This is
 * true no matter what sub-index the read begins at.
 *
 * input parameters
 * @param buffer - the buffer into which the data will be read
 * @param length - the length of data to read (in bytes)
 * @param accOffset - the offset in the acc buffer from which to read the data,
 * this is a complex sample index e.g. to read 10 samples starting at sample 100
...
void dwt_readaccdata(uint8_t *buffer, uint16_t length, uint16_t accOffset) {
  ...
  dwt_or16bitoffsetreg(CLK_CTRL_ID, 0x0,
                       CLK_CTRL_ACC_MCLK_EN_BIT_MASK |
                           CLK_CTRL_ACC_CLK_EN_BIT_MASK);
  ...
  dwt_and16bitoffsetreg(CLK_CTRL_ID, 0x0,
                        (uint16_t) ~(CLK_CTRL_ACC_MCLK_EN_BIT_MASK |
                                     CLK_CTRL_ACC_CLK_EN_BIT_MASK));
}
 ...
 * offset of the far DW3000 device compared to this one) Note: the returned
 * signed 16-bit number should be divided by by 2^26 to get ppm offset.
 *
...
int16_t dwt_readclockoffset(void) {
  ...
  switch (pdw3000local->dblbuffon) // if the flag is non zero - we are either
                                   // accessing RX_BUFFER_0 or RX_BUFFER_1
  {
  case DBL_BUFF_ACCESS_BUFFER_1:
    //!!! Assumes that Indirect pointer register B was already set. This is done
    //!in the dwt_setdblrxbuffmode when mode is enabled.
    regval = dwt_read16bitoffsetreg(INDIRECT_POINTER_B_ID,
                                    (BUF1_CIA_DIAG_0 - BUF1_RX_FINFO)) &
             CIA_DIAG_0_COE_PPM_BIT_MASK;
    break;
  case DBL_BUFF_ACCESS_BUFFER_0:
    regval = dwt_read16bitoffsetreg(BUF0_CIA_DIAG_0, 0) &
             CIA_DIAG_0_COE_PPM_BIT_MASK;
    break;
  default:
    regval =
        dwt_read16bitoffsetreg(CIA_DIAG_0_ID, 0) & CIA_DIAG_0_COE_PPM_BIT_MASK;
    break;
  }
 ...
 * significant bit, which is bit #20 (numbering from bit zero) to return a
 * 32-bit signed integer value.
 *
 * input parameters - NONE
 *
 * return value - the (int32_t) signed carrier integrator value.
 *                A positive value means the local RX clock is running slower
 * than the remote TX device.
 */
int32_t dwt_readcarrierintegrator(void) {
  ...
  for (j = 2; j >= 0;
       j--) // arrange the three bytes into an unsigned integer value
  {
    regval = (regval << 8) + buffer[j];
  }
 ...
 * assume good STS reception. Otherwise the STS timestamp may not be accurate.
 */
int dwt_readstsquality(int16_t *rxStsQualityIndex) {
  ...
  preambleCount =
      dwt_read16bitoffsetreg(STS_STS_ID, 0) &
      STS_STS_ACC_QUAL_BIT_MASK; //  dwt_read16bitoffsetreg(CP_PRNG_ID,
                                 //  CP_STS_OFFSET) & CP_ACC_CP_QUAL_MASK;
  ...
  *rxStsQualityIndex = (int16_t)preambleCount;
  ...
  // determine if the STS Rx quality is good or bad (return >=0 for good and < 0
  // if bad)
 ...
 *
 * input parameters
 * @param stsStatus - the (uint16_t) STS status value. 9 bits of this buffer are
 * populated with various STS statuses. The remaining 7 bits are ignored.
 * @param sts_num   - 0 for 1st STS, 1 for 2nd STS (2nd only valid when PDOA
 * Mode 3 is used)
 *
 * output parameters
 * return value DWT_SUCCESS for good/valid STS status, DWT_ERROR if bad STS
 * status.
 */
int dwt_readstsstatus(uint16_t *stsStatus, int sts_num) {
  ...
  switch (pdw3000local->dblbuffon) // check if in double buffer mode and if so
                                   // which buffer host is currently accessing
  {
  case DBL_BUFF_ACCESS_BUFFER_1:
    //!!! Assumes that Indirect pointer register B was already set. This is done
    //!in the dwt_setdblrxbuffmode when mode is enabled.
    *stsStatus =
        dwt_read16bitoffsetreg(INDIRECT_POINTER_B_ID,
                               (stsStatusRegAdd - BUF0_RX_FINFO + 2)) >>
        7;
    break;
  case DBL_BUFF_ACCESS_BUFFER_0:
...
void dwt_readdiagnostics(dwt_rxdiag_t *diagnostics) {
  ...
  uint8_t
      temp[DB_MAX_DIAG_SIZE]; // address from 0xC0000 to 0xD0068 (108*2 bytes) -
                              // when using normal mode, or 232 length for max
                              // logging when in Double Buffer mode
  ...
  case DBL_BUFF_ACCESS_BUFFER_0:
    ...
    if (pdw3000local->dblbuffon == DBL_BUFF_ACCESS_BUFFER_1) {
      /* Program the indirect offset registers B for specified offset to
       * swinging set buffer B */
      //!!! Assumes that Indirect pointer register B was already set. This is
      //!done in the dwt_setdblrxbuffmode when mode is enabled.
      /* Indirectly read data from the IC to the buffer */
    ...
    for (i = 0; i < (CIA_I_RX_TIME_LEN + 1); i++) {
      diagnostics->tdoa[i] =
          temp[i + BUF0_TDOA - BUF0_RX_FINFO]; // timestamp difference of the 2
                                               // STS RX timestamps
    }

    diagnostics->xtalOffset =
        ((int16_t)temp[BUF0_CIA_DIAG_0 - BUF0_RX_FINFO + 1] << 8 |
         temp[BUF0_CIA_DIAG_0 - BUF0_RX_FINFO]) &
        0x1FFF; // Estimated xtal offset of remote device

    diagnostics->pdoa =
        ((int16_t)temp[BUF0_PDOA - BUF0_RX_FINFO + 3] << 8 |
         temp[BUF0_PDOA - BUF0_RX_FINFO + 2]) &
    ...
    diagnostics->ipatovAccumCount =
        ((uint16_t)temp[BUF0_IP_DIAG_12 - BUF0_RX_FINFO + 1] << 8 |
         temp[BUF0_IP_DIAG_12 - BUF0_RX_FINFO]) &
    ...
    for (i = 0; i < CIA_I_RX_TIME_LEN; i++) {
      diagnostics->ipatovRxTime[i] =
          temp[i + BUF0_IP_TS -
               BUF0_RX_FINFO]; // RX timestamp from Ipatov sequence
      diagnostics->stsRxTime[i] =
          temp[i + BUF0_STS_TS - BUF0_RX_FINFO]; // RX timestamp from STS
      diagnostics->sts2RxTime[i] =
          temp[i + BUF0_STS1_TS - BUF0_RX_FINFO]; // RX timestamp from STS1
    }
    diagnostics->ipatovRxStatus =
        temp[BUF0_IP_TS - BUF0_RX_FINFO +
             CIA_C_STAT_OFFSET]; // RX status info for Ipatov sequence
    diagnostics->ipatovPOA = (uint16_t)temp[BUF0_IP_TS - BUF0_RX_FINFO + 2]
                                 << 8 |
                             temp[BUF0_IP_TS - BUF0_RX_FINFO +
                                  1]; // Phase of arrival as computed from the
                                      // Ipatov CIR (signed rad*2-12)

    diagnostics->stsRxStatus =
        temp[BUF0_STS_TS - BUF0_RX_FINFO +
             CIA_C_STAT_OFFSET]; // RX status info for STS
    diagnostics->stsPOA = (uint16_t)temp[BUF0_STS_TS - BUF0_RX_FINFO + 2] << 8 |
                          temp[BUF0_STS_TS - BUF0_RX_FINFO +
                               1]; // Phase of arrival as computed from the STS
                                   // 1 CIR (signed rad*2-12)

    diagnostics->sts2RxStatus =
        temp[BUF0_STS1_TS - BUF0_RX_FINFO +
             CIA_C_STAT_OFFSET]; // RX status info for STS1
    diagnostics->sts2POA = (uint16_t)temp[BUF0_STS1_TS - BUF0_RX_FINFO + 2]
                               << 8 |
                           temp[BUF0_STS1_TS - BUF0_RX_FINFO +
                                1]; // Phase of arrival as computed from the STS
                                    // 1 CIR (signed rad*2-12)
    ...
    diagnostics->ciaDiag1 =
        ((uint32_t)temp[BUF0_CIA_DIAG_1 - BUF0_RX_FINFO + 3] << 24 |
        ...
        0x1FFFFFFF; // Diagnostics common to both sequences (carrier integrator
                    // [28:8] and resampler delay [7:0])

    // IP
    diagnostics->ipatovPeak =
        ((uint32_t)temp[BUF0_IP_DIAG_0 - BUF0_RX_FINFO + 3] << 24 |
        ...
        0x7FFFFFFF; // index [30:21] and amplitude [20:0] of peak sample in
                    // Ipatov sequence CIR
    diagnostics->ipatovPower =
        ((uint32_t)temp[BUF0_IP_DIAG_1 - BUF0_RX_FINFO + 3] << 24 |
        ...
        0x1FFFF; // channel area allows estimation [16:0] of channel power for
                 // the Ipatov sequence
    diagnostics->ipatovF1 =
        ((uint32_t)temp[BUF0_IP_DIAG_2 - BUF0_RX_FINFO + 3] << 24 |
    ...
    diagnostics->ipatovF2 =
        ((uint32_t)temp[BUF0_IP_DIAG_3 - BUF0_RX_FINFO + 3] << 24 |
    ...
    diagnostics->ipatovF3 =
        ((uint32_t)temp[BUF0_IP_DIAG_4 - BUF0_RX_FINFO + 3] << 24 |
    ...
    diagnostics->ipatovFpIndex =
        (uint16_t)temp[BUF0_IP_DIAG_8 - BUF0_RX_FINFO + 1] << 8 |
        temp[BUF0_IP_DIAG_8 -
             BUF0_RX_FINFO]; // First path index [15:0] for Ipatov sequence

    // CP 1
    diagnostics->stsPeak =
        ((uint32_t)temp[BUF0_STS_DIAG_0 - BUF0_RX_FINFO + 3] << 24 |
        ...
        0x3FFFFFFF; // index [29:21] and amplitude [20:0] of peak sample in STS
                    // CIR
    diagnostics->stsPower =
        ((uint16_t)temp[BUF0_STS_DIAG_1 - BUF0_RX_FINFO + 1] << 8 |
         temp[BUF0_STS_DIAG_1 -
              BUF0_RX_FINFO]); // channel area allows estimation of channel
                               // power for the STS
    diagnostics->stsF1 =
        ((uint32_t)temp[BUF0_STS_DIAG_2 - BUF0_RX_FINFO + 3] << 24 |
    ...
    diagnostics->stsF2 =
        ((uint32_t)temp[BUF0_STS_DIAG_3 - BUF0_RX_FINFO + 3] << 24 |
    ...
    diagnostics->stsF3 =
        ((uint32_t)temp[BUF0_STS_DIAG_4 - BUF0_RX_FINFO + 3] << 24 |
    ...
    diagnostics->stsFpIndex =
        ((uint16_t)temp[BUF0_STS_DIAG_8 - BUF0_RX_FINFO + 1] << 8 |
         temp[BUF0_STS_DIAG_8 - BUF0_RX_FINFO]) &
        0x7FFF; // First path index [14:0] for STS
    diagnostics->stsAccumCount =
        ((uint16_t)temp[BUF0_STS_DIAG_12 - BUF0_RX_FINFO + 1] << 8 |
         temp[BUF0_STS_DIAG_12 - BUF0_RX_FINFO]) &
        0xFFF; // Number accumulated symbols [11:0] for STS

    // CP 2
    diagnostics->sts2Peak =
        ((uint32_t)temp[BUF0_STS1_DIAG_0 - BUF0_RX_FINFO + 3] << 24 |
        ...
        0x3FFFFFFF; // index [29:21] and amplitude [20:0] of peak sample in STS
                    // CIR
    diagnostics->sts2Power =
        ((uint16_t)temp[BUF0_STS1_DIAG_1 - BUF0_RX_FINFO + 1] << 8 |
         temp[BUF0_STS1_DIAG_1 -
              BUF0_RX_FINFO]); // channel area allows estimation of channel
                               // power for the STS
    diagnostics->sts2F1 =
        ((uint32_t)temp[BUF0_STS1_DIAG_2 - BUF0_RX_FINFO + 3] << 24 |
        ...
        0x3FFFFF; // F1 for STS [21:0]
    diagnostics->sts2F2 =
        ((uint32_t)temp[BUF0_STS1_DIAG_3 - BUF0_RX_FINFO + 3] << 24 |
        ...
        0x3FFFFF; // F2 for STS [21:0]
    diagnostics->sts2F3 =
        ((uint32_t)temp[BUF0_STS1_DIAG_4 - BUF0_RX_FINFO + 3] << 24 |
        ...
        0x3FFFFF; // F3 for STS [21:0]
    diagnostics->sts2FpIndex =
        ((uint16_t)temp[BUF0_STS1_DIAG_8 - BUF0_RX_FINFO + 1] << 8 |
         temp[BUF0_STS1_DIAG_8 - BUF0_RX_FINFO]) &
        0x7FFF; // First path index [14:0] for STS
    diagnostics->sts2AccumCount =
        ((uint16_t)temp[BUF0_STS1_DIAG_12 - BUF0_RX_FINFO + 1] << 8 |
         temp[BUF0_STS1_DIAG_12 - BUF0_RX_FINFO]) &
        0xFFF; // Number accumulated symbols [11:0] for STS
  ...
  default: // double buffer is off
    ...
    if (pdw3000local->cia_diagnostic & DW_CIA_DIAG_LOG_ALL) {
      dwt_readfromdevice(IP_TOA_LO_ID, 0, 108,
                         temp); // read form 0xC0000 space  (108 bytes)
      dwt_readfromdevice(STS_DIAG_4_ID, 0, 108,
                         &temp[108]); // read from 0xD0000 space  (108 bytes)
    } else {
    ...
    for (i = 0; i < CIA_I_RX_TIME_LEN; i++) {
      diagnostics->ipatovRxTime[i] =
          temp[i]; // RX timestamp from Ipatov sequence
      diagnostics->stsRxTime[i] =
          temp[i + STS_TOA_LO_ID - IP_TOA_LO_ID]; // RX timestamp from STS
      diagnostics->sts2RxTime[i] =
          temp[i + STS1_TOA_LO_ID - IP_TOA_LO_ID]; // RX timestamp from STS1
      diagnostics->tdoa[i] =
          temp[i + CIA_TDOA_0_ID -
               IP_TOA_LO_ID]; // timestamp difference of the 2 STS RX timestamps
    }
    diagnostics->tdoa[5] = temp[5 + CIA_TDOA_0_ID - IP_TOA_LO_ID];

    diagnostics->ipatovRxStatus =
        temp[IP_TOA_HI_ID - IP_TOA_LO_ID +
             CIA_I_STAT_OFFSET]; // RX status info for Ipatov sequence
    diagnostics->ipatovPOA = (uint16_t)temp[IP_TOA_HI_ID - IP_TOA_LO_ID + 2]
                                 << 8 |
                             temp[IP_TOA_HI_ID - IP_TOA_LO_ID +
                                  1]; // Phase of arrival as computed from the
                                      // Ipatov CIR (signed rad*2-12)
    ...
    diagnostics->stsRxStatus =
        ((uint16_t)temp[STS_TOA_HI_ID - IP_TOA_LO_ID + CIA_C_STAT_OFFSET + 1] |
         temp[STS_TOA_HI_ID - IP_TOA_LO_ID + CIA_C_STAT_OFFSET]) >>
        7; // RX status info for STS
    diagnostics->stsPOA = (uint16_t)temp[STS_TOA_HI_ID - IP_TOA_LO_ID + 2]
                              << 8 |
                          temp[STS_TOA_HI_ID - IP_TOA_LO_ID +
                               1]; // Phase of arrival as computed from the STS
                                   // 1 CIR (signed rad*2-12)

    diagnostics->sts2RxStatus =
        ((uint16_t)temp[STS1_TOA_HI_ID - IP_TOA_LO_ID + CIA_C_STAT_OFFSET + 1] |
         temp[STS_TOA_HI_ID - IP_TOA_LO_ID + CIA_C_STAT_OFFSET]) >>
        7; // RX status info for STS
    diagnostics->sts2POA = (uint16_t)temp[STS1_TOA_HI_ID - IP_TOA_LO_ID + 2]
                               << 8 |
                           temp[STS1_TOA_HI_ID - IP_TOA_LO_ID +
                                1]; // Phase of arrival as computed from the STS
                                    // 1 CIR (signed rad*2-12)

    diagnostics->pdoa =
        ((int16_t)temp[CIA_TDOA_1_PDOA_ID - IP_TOA_LO_ID + 3] << 8 |
         temp[CIA_TDOA_1_PDOA_ID - IP_TOA_LO_ID + 2]) &
        0x3FFF; // phase difference of the 2 STS POAs (signed in [1:-11])
    if (diagnostics->pdoa & 0x2000)
      diagnostics->pdoa |= 0xC000; // sign extend

    diagnostics->xtalOffset =
        ((int16_t)temp[CIA_DIAG_0_ID - IP_TOA_LO_ID + 1] << 8 |
         temp[CIA_DIAG_0_ID - IP_TOA_LO_ID]) &
        0x1FFF; // Estimated xtal offset of remote device

    diagnostics->ciaDiag1 =
        ((uint32_t)temp[CIA_DIAG_1_ID - IP_TOA_LO_ID + 3] << 24 |
        ...
        0x1FFFFFFF; // Diagnostics common to both sequences
    ...
    // IP
    diagnostics->ipatovPeak =
        ((uint32_t)temp[IP_DIAG_0_ID - IP_TOA_LO_ID + 3] << 24 |
        ...
        0x7FFFFFFF; // index [30:21] and amplitude [20:0] of peak sample in
                    // Ipatov sequence CIR
    diagnostics->ipatovPower =
        ((uint32_t)temp[IP_DIAG_1_ID - IP_TOA_LO_ID + 3] << 24 |
    ...
    diagnostics->ipatovF3 =
        ...
        0x3FFFFF; // F3 for Ipatov sequence [21:0]
    diagnostics->ipatovFpIndex =
        (uint16_t)temp[IP_DIAG_8_ID - IP_TOA_LO_ID + 1] << 8 |
        temp[IP_DIAG_8_ID -
             IP_TOA_LO_ID]; // First path index [15:0] for Ipatov sequence
    diagnostics->ipatovAccumCount =
        ((uint16_t)temp[IP_DIAG_12_ID - IP_TOA_LO_ID + 1] << 8 |
         temp[IP_DIAG_12_ID - IP_TOA_LO_ID]) &
        0xFFF; // Number accumulated symbols [11:0] for Ipatov sequence

    // STS 1
    diagnostics->stsPeak =
        ((uint32_t)temp[STS_DIAG_0_ID - IP_TOA_LO_ID + 3] << 24 |
         (uint32_t)temp[STS_DIAG_0_ID - IP_TOA_LO_ID + 2] << 16 |
         (uint32_t)temp[STS_DIAG_0_ID - IP_TOA_LO_ID + 1] << 8 |
         (uint32_t)temp[STS_DIAG_0_ID - IP_TOA_LO_ID]) &
        0x3FFFFFFF; // index [29:21] and amplitude [20:0] of peak sample in STS
                    // CIR
    diagnostics->stsPower =
        ((uint16_t)temp[STS_DIAG_1_ID - IP_TOA_LO_ID + 1] << 8 |
         temp[STS_DIAG_1_ID - IP_TOA_LO_ID]); // channel area allows estimation
                                              // of channel power for the STS
    diagnostics->stsF1 =
        ((uint32_t)temp[STS_DIAG_2_ID - IP_TOA_LO_ID + 3] << 24 |
         (uint32_t)temp[STS_DIAG_2_ID - IP_TOA_LO_ID + 2] << 16 |
         (uint32_t)temp[STS_DIAG_2_ID - IP_TOA_LO_ID + 1] << 8 |
         (uint32_t)temp[STS_DIAG_2_ID - IP_TOA_LO_ID]) &
        0x3FFFFF; // F1 for STS [21:0]
    diagnostics->stsF2 =
        ((uint32_t)temp[STS_DIAG_3_ID - IP_TOA_LO_ID + 3] << 24 |
         (uint32_t)temp[STS_DIAG_3_ID - IP_TOA_LO_ID + 2] << 16 |
         (uint32_t)temp[STS_DIAG_3_ID - IP_TOA_LO_ID + 1] << 8 |
         (uint32_t)temp[STS_DIAG_3_ID - IP_TOA_LO_ID]) &
        0x3FFFFF; // F2 for STS [21:0]

    offset_0xd =
        0x6c; // there are 0x6C bytes in 0xC0000 base before we enter 0xD0000

    diagnostics->stsF3 =
        ((uint32_t)temp[STS_DIAG_4_ID - STS_DIAG_4_ID + offset_0xd + 3] << 24 |
         (uint32_t)temp[STS_DIAG_4_ID - STS_DIAG_4_ID + offset_0xd + 2] << 16 |
         (uint32_t)temp[STS_DIAG_4_ID - STS_DIAG_4_ID + offset_0xd + 1] << 8 |
         (uint32_t)temp[STS_DIAG_4_ID - STS_DIAG_4_ID + offset_0xd]) &
        0x3FFFFF; // F3 for STS [21:0]
    diagnostics->stsFpIndex =
        ((uint16_t)temp[STS_DIAG_8_ID - STS_DIAG_4_ID + offset_0xd + 1] << 8 |
         temp[STS_DIAG_8_ID - STS_DIAG_4_ID + offset_0xd]) &
        0x7FFF; // First path index [14:0] for STS
    diagnostics->stsAccumCount =
        ((uint16_t)temp[STS_DIAG_12_ID - STS_DIAG_4_ID + offset_0xd + 1] << 8 |
         temp[STS_DIAG_12_ID - STS_DIAG_4_ID + offset_0xd]) &
        0xFFF; // Number accumulated symbols [11:0] for STS

    // STS 2
    diagnostics->sts2Peak =
        ((uint32_t)temp[STS1_DIAG_0_ID - STS_DIAG_4_ID + offset_0xd + 3] << 24 |
         (uint32_t)temp[STS1_DIAG_0_ID - STS_DIAG_4_ID + offset_0xd + 2] << 16 |
         (uint32_t)temp[STS1_DIAG_0_ID - STS_DIAG_4_ID + offset_0xd + 1] << 8 |
         (uint32_t)temp[STS1_DIAG_0_ID - STS_DIAG_4_ID + offset_0xd]) &
        0x3FFFFFFF; // index [29:21] and amplitude [20:0] of peak sample in STS
                    // CIR
    diagnostics->sts2Power =
        ((uint16_t)temp[STS1_DIAG_1_ID - STS_DIAG_4_ID + offset_0xd + 1] << 8 |
         temp[STS1_DIAG_1_ID - STS_DIAG_4_ID +
              offset_0xd]); // channel area allows estimation of channel power
                            // for the STS
    diagnostics->sts2F1 =
        ((uint32_t)temp[STS1_DIAG_2_ID - STS_DIAG_4_ID + offset_0xd + 3] << 24 |
         (uint32_t)temp[STS1_DIAG_2_ID - STS_DIAG_4_ID + offset_0xd + 2] << 16 |
         (uint32_t)temp[STS1_DIAG_2_ID - STS_DIAG_4_ID + offset_0xd + 1] << 8 |
         (uint32_t)temp[STS1_DIAG_2_ID - STS_DIAG_4_ID + offset_0xd]) &
        0x3FFFFF; // F1 for STS [21:0]
    diagnostics->sts2F2 =
        ((uint32_t)temp[STS1_DIAG_3_ID - STS_DIAG_4_ID + offset_0xd + 3] << 24 |
         (uint32_t)temp[STS1_DIAG_3_ID - STS_DIAG_4_ID + offset_0xd + 2] << 16 |
         (uint32_t)temp[STS1_DIAG_3_ID - STS_DIAG_4_ID + offset_0xd + 1] << 8 |
         (uint32_t)temp[STS1_DIAG_3_ID - STS_DIAG_4_ID + offset_0xd]) &
        0x3FFFFF; // F2 for STS [21:0]
    diagnostics->sts2F3 =
        ((uint32_t)temp[STS1_DIAG_4_ID - STS_DIAG_4_ID + offset_0xd + 3] << 24 |
         (uint32_t)temp[STS1_DIAG_4_ID - STS_DIAG_4_ID + offset_0xd + 2] << 16 |
         (uint32_t)temp[STS1_DIAG_4_ID - STS_DIAG_4_ID + offset_0xd + 1] << 8 |
         (uint32_t)temp[STS1_DIAG_4_ID - STS_DIAG_4_ID + offset_0xd]) &
        0x3FFFFF; // F3 for STS [21:0]
    diagnostics->sts2FpIndex =
        ((uint16_t)temp[STS1_DIAG_8_ID - STS_DIAG_4_ID + offset_0xd + 1] << 8 |
         temp[STS1_DIAG_8_ID - STS_DIAG_4_ID + offset_0xd]) &
        0x7FFF; // First path index [14:0] for STS
    diagnostics->sts2AccumCount =
        ((uint16_t)temp[STS1_DIAG_12_ID - STS_DIAG_4_ID + offset_0xd + 1] << 8 |
         temp[STS1_DIAG_12_ID - STS_DIAG_4_ID + offset_0xd]) &
        0xFFF; // Number accumulated symbols [11:0] for STS
    break;
  }
}
...
void dwt_readtxtimestamp(uint8_t *timestamp) {
  dwt_readfromdevice(TX_TIME_LO_ID, 0, TX_TIME_TX_STAMP_LEN,
...
uint32_t dwt_readtxtimestamphi32(void) {
  return dwt_read32bitoffsetreg(
      TX_TIME_LO_ID, 1); // Offset is 1 to get the 4 upper bytes out of 5
}
...
uint32_t dwt_readtxtimestamplo32(void) {
  return dwt_read32bitreg(TX_TIME_LO_ID); // Read TX TIME as a 32-bit register
                                          // to get the 4 lower bytes out of 5
}

/*!
 * ------------------------------------------------------------------------------------------------------------------
 * @brief This is used to read the PDOA result, it is the phase difference
 * between either the Ipatov and STS POA (in PDOA mode 1), or the two STS POAs
 * (in PDOA mode 3), depending on the PDOA mode of operation. (POA - Phase Of
 * Arrival)
 *
...
int16_t dwt_readpdoa(void) {
  ...
  default:
    pdoa =
        dwt_read16bitoffsetreg(CIA_TDOA_1_PDOA_ID, 2) &
 ...
 * The TDOA value that is read from the register is 41-bits in length. However,
 * 6 bytes (or 48 bits) are read from the register. The remaining 7 bits at the
 * 'top' of the 6 bytes that are not part of the TDOA value should be set to
 * zero and should not interfere with rest of the 41-bit value. However, there
 * is no harm in masking the returned value.
 *
 * input parameters
 *
 * output parameters
...
void dwt_readtdoa(uint8_t *tdoa) {
  ...
  tdoa[5] &= 0x01; // TDOA value is 41 bits long. You will need to read 6 bytes
                   // and mask the highest byte with 0x01
}
...
void dwt_readrxtimestamp(uint8_t *timestamp) {
  ...
  case DBL_BUFF_ACCESS_BUFFER_0:
    ...
    break;
  default:
    dwt_readfromdevice(RX_TIME_0_ID, 0, RX_TIME_RX_STAMP_LEN,
...
/*!
 * ------------------------------------------------------------------------------------------------------------------
 * @brief This is used to read the raw RX timestamp (RMARKER time) before any
 * CIA first path analysis adjustments
 *
 * input parameters
 * @param timestamp - a pointer to a 4-byte buffer which will store the read RX
 * timestamp time
 *
 * output parameters - the timestamp buffer will contain the value after the
 * function call
...
void dwt_readrxtimestamp_ipatov(uint8_t *timestamp) {
  ...
  default:
    dwt_readfromdevice(
        IP_TOA_LO_ID, 0, CIA_I_RX_TIME_LEN,
        timestamp); // Get the adjusted time of arrival w.r.t. Ipatov CIR
    break;
  }
}
...
void dwt_readrxtimestamp_sts(uint8_t *timestamp) {
  ...
  default:
    dwt_readfromdevice(
        STS_TOA_LO_ID, 0, CIA_C_RX_TIME_LEN,
        timestamp); // Get the adjusted time of arrival w.r.t. STS CIR
    break;
  }
}
...
uint32_t dwt_readrxtimestamphi32(void) {
  return dwt_read32bitoffsetreg(
      RX_TIME_0_ID,
      1); // Offset is 1 to get the 4 upper bytes out of 5 byte tiemstamp
}
...
uint32_t dwt_readrxtimestamplo32(void) {
  return dwt_read32bitreg(
      RX_TIME_0_ID); // Read RX TIME as a 32-bit register to get the 4 lower
                     // bytes out of 5 byte timestamp
}
 ...
 * configures 802.15.4 type DWT_FF_ENABLE_802_15_4      0x2             // use
 * 802.15.4 filtering rules DWT_FF_DISABLE              0x0             //
 * disable FF
 * @param filtermode (bitmask) - configures the frame filtering options
 * according to DWT_FF_BEACON_EN            0x001           // beacon frames
 * allowed DWT_FF_DATA_EN              0x002           // data frames allowed
 *       DWT_FF_ACK_EN               0x004           // ack frames allowed
 *       DWT_FF_MAC_EN               0x008           // mac control frames
 * allowed DWT_FF_RSVD_EN              0x010           // reserved frame types
 * allowed DWT_FF_MULTI_EN             0x020           // multipurpose frames
 * allowed DWT_FF_FRAG_EN              0x040           // fragmented frame types
 * allowed DWT_FF_EXTEND_EN            0x080           // extended frame types
 * allowed DWT_FF_COORD_EN             0x100           // behave as coordinator
 * (can receive frames with no dest address (PAN ID has to match))
 *       DWT_FF_IMPBRCAST_EN         0x200           // allow MAC implicit
 * broadcast DWT_FF_LE0_PEND             0x400           // Data pending for
 * device at LE0 address. see dwt_configure_le_address for more info
 *       DWT_FF_LE1_PEND             0x800           // Data pending for device
 * at LE1 address. see dwt_configure_le_address for more info DWT_FF_LE2_PEND
 * 0x1000           // Data pending for device at LE2 address. see
 * dwt_configure_le_address for more info DWT_FF_LE3_PEND             0x2000 //
 * Data pending for device at LE3 address. see dwt_configure_le_address for more
 * info DWT_SSADRAPE                0x4000          //Short Source Address Data
 * Request ACK with PEND Enable DWT_LSADRAPE                0x8000 //Long Source
 * Address Data Request ACK with PEND Enable output parameters
 *
 * no return value
 */
...
uint8_t dwt_aon_read(uint16_t aon_address) {
  dwt_write16bitoffsetreg(AON_ADDR_ID, 0x0,
                          aon_address); // Set short AON address for read
  ...
  return dwt_read8bitoffsetreg(AON_RDATA_ID,
                               0x0); // Return the data that was read
}
...
void dwt_aon_write(uint16_t aon_address, uint8_t aon_write_data) {
  ...
  dwt_write16bitoffsetreg(AON_ADDR_ID, 0x0,
                          aon_address); // Set AON address for write
  dwt_write8bitoffsetreg(AON_WDATA_ID, 0x0, aon_write_data); // Set write data
  dwt_write8bitoffsetreg(AON_CTRL_ID, 0x0,
                         (temp | AON_CTRL_DCA_ENAB_BIT_MASK |
                          AON_CTRL_DCA_WRITE_EN_BIT_MASK)); // Enable write
  dwt_write8bitoffsetreg(AON_CTRL_ID, 0x0, 0x0); // Clear all enabled bits
}
...
/*!
 * ------------------------------------------------------------------------------------------------------------------
 * @brief For each value to send to OTP bloc, following two register writes are
...
void _dwt_otpprogword32(uint32_t data, uint16_t address) {
  ...
  */
  ...
  The for loop will exit once the status indicates programming is complete or if
  it reaches the max 1000 iterations. 1000 is more than sufficient for max OTP
  programming delay and max supported DW3000 SPI rate. Instead a delay of 2ms
  (as commented out below) can be used. Burn time is about 1.76ms
  */
  ...
  deca_sleep(2); // Uncomment this command if you don't want to use the loop
                 // above. It will take more time than the loop above.
 ...
 * dwt_configuresleep() should be called first to configure the sleep and
 * on-wake/wake-up parameters
 *
 * input parameters
 * @param idle_rc - if this is set to DWT_DW_IDLE_RC, the auto INIT2IDLE bit
 * will be cleared prior to going to sleep thus after wake-up device will stay
 * in IDLE_RC state
 *
 * output parameters
 *
...
/*!
 * ------------------------------------------------------------------------------------------------------------------
 * @brief sets the sleep counter to new value, this function programs the sleep
 * counter top 16-bits [27:12]
 *
...
uint16_t dwt_calibratesleepcnt(void) {
  ...
  deca_sleep(2); // need to wait for at least 1 LP OSC period at slowest
                 // frequency of 15kHz =~ 66 us
 ...
 * mode i.e. before entering the sleep, the device should be programmed for TX
 * or RX, then upon "waking up" the TX/RX settings will be preserved and the
 * device can immediately perform the desired action TX/RX
 *
 ...
 * although it will self-clear on wake up DWT_WAKE_WK      0x10 - wake up on
 * WAKEUP PIN DWT_WAKE_CS      0x8 - wake up on chip select DWT_BR_DET       0x4
 * - enable brownout detector during sleep/deep sleep DWT_SLEEP        0x2 -
 * enable sleep DWT_SLP_EN       0x1 - enable sleep/deep sleep functionality
 *
 * input parameters
...
void dwt_configuresleep(uint16_t mode, uint8_t wake) {
  ...
  dwt_write8bitoffsetreg(
      ANA_CFG_ID, 0,
      wake); // bit 0 - SLEEP_EN, bit 1 - DEEP_SLEEP=0/SLEEP=1, bit 3 wake on CS
}
 ...
 * transmission the device will enter deep sleep mode. The dwt_configuresleep()
 * function needs to be called before this to configure the on-wake settings
 *
 ...
 * following registers will be logged: IP_TOA_LO, IP_TOA_HI, STS_TOA_LO,
 * STS_TOA_HI, STS1_TOA_LO, STS1_TOA_HI, CIA_TDOA_0, CIA_TDOA_1_PDOA,
 * CIA_DIAG_0, CIA_DIAG_1
 *
 * input parameters
 ...
 * diagnostic registers DW_CIA_DIAG_LOG_MIN (0x0)   //CIA to log reduced set of
 ...
 * (parameter) is 0, the ACK will be sent a.s.a.p. otherwise it will be sent
 * with a programmed delay (in symbols), max is 255. NOTE: needs to have frame
 * filtering enabled as well
 *
 * input parameters
...
void dwt_enableautoack(uint8_t responseDelayTime, int enable) {
  ...
  // Enable AUTO ACK
  if (enable) {
    dwt_or32bitoffsetreg(
        SYS_CFG_ID, 0,
        SYS_CFG_AUTO_ACK_BIT_MASK |
            SYS_CFG_FAST_AAT_EN_BIT_MASK); // set the AUTO_ACK bit
  } else {
    dwt_and16bitoffsetreg(
        SYS_CFG_ID, 0,
...
/*!
 * ------------------------------------------------------------------------------------------------------------------
 * @brief This API sends issues a command to the device that the specific RX
 * buff is free for frame reception, it will also update the dblbuffon
 * flag/status to the next buffer
 *
 * input parameters
 * @param None
 *
...
void dwt_signal_rx_buff_free(void) {
  ...
  if (pdw3000local->dblbuffon == DBL_BUFF_ACCESS_BUFFER_1) {
    pdw3000local->dblbuffon =
        DBL_BUFF_ACCESS_BUFFER_0; // next buffer is RX_BUFFER_0
  } else {
    pdw3000local->dblbuffon =
        DBL_BUFF_ACCESS_BUFFER_1; // next buffer is RX_BUFFER_1
  }
}
...
void dwt_setdblrxbuffmode(dwt_dbl_buff_state_e dbl_buff_state,
  ...
  if (dbl_buff_state == DBL_BUF_STATE_EN) {
    ...
    pdw3000local->dblbuffon =
        DBL_BUFF_ACCESS_BUFFER_0; // the host will access RX_BUFFER_0 initially
                                  // (on 1st reception after enable)
    // Updating indirect address here to save time setting it inside the
    // interrupt(in order to read BUF1_RX_FINFO).. Pay attention that after
    // sleep, this register needs to be set again.
    dwt_write32bitreg(INDIRECT_ADDR_B_ID, (BUF1_RX_FINFO >> 16));
    dwt_write32bitreg(ADDR_OFFSET_B_ID, BUF1_RX_FINFO & 0xffff);
  } else {
...
void dwt_setrxaftertxdelay(uint32_t rxDelayTime) {
  ...
  val |= (rxDelayTime &
          ACK_RESP_W4R_TIM_BIT_MASK); // In UWB microseconds (e.g. turn the
                                      // receiver on 20uus after TX)
 ...
 * process the event as usual but the 'null' callback will not be called.
 *
 * input parameters
...
uint8_t dwt_checkidlerc(void) {
  ...
  /* Poll DW IC until IDLE_RC event set. This means that DW IC is in IDLE_RC
   * state and ready */
 ...
 *          - RXFR + no data mode (through cbRxOk callback, but set datalength
 to 0)
 *          - RXFCG (through cbRxOk callback)
 *          - TXFRS (through cbTxDone callback)
 *          - RXRFTO/RXPTO (through cbRxTo callback)
 *          - RXPHE/RXFCE/RXRFSL/RXSFDTO/AFFREJ/LDEERR/LCSSERR (through cbRxTo
 cbRxErr)
 *          -
...
void dwt_isr(void) {
  ...
  if ((pdw3000local->stsconfig & DWT_STS_MODE_ND) ==
      DWT_STS_MODE_ND) // cannot use FSTAT when in no data mode...
  {
  ...
  if (fstat & FINT_STAT_SYS_PANIC_BIT_MASK) {
    ...
    // Handle SPI error events (if this has happened, the last SPI transaction
    // has not completed correctly, the device should be reset)
    ...
    // Handle Fast CMD errors event, the means the last CMD did not execute
    // (e.g. it was given while device was already executing previous)
  ...
  if (fstat & FINT_STAT_TXOK_BIT_MASK) {
    // Clear TX events after the callback - this lets the host schedule another
    // TX/RX inside the callback
    dwt_write8bitoffsetreg(
        SYS_STATUS_ID, 0,
        (uint8_t)
            SYS_STATUS_ALL_TX); // Clear TX event bits to clear the interrupt
  ...
  if (fstat & FINT_STAT_SYS_EVENT_BIT_MASK) {
    ...
    // SYS_STATUS register inside the callback
    dwt_write16bitoffsetreg(
        SYS_STATUS_ID, 2,
        (uint16_t)((SYS_STATUS_RCINIT_BIT_MASK | SYS_STATUS_SPIRDY_BIT_MASK) >>
                   16)); // Clear the bit to clear the interrupt
  ...
  if (fstat & FINT_STAT_RXOK_BIT_MASK) {
    ...
    if (pdw3000local->dblbuffon) // if in double buffer mode
    {
      ...
      if (pdw3000local->dblbuffon ==
          DBL_BUFF_ACCESS_BUFFER_1) // If accessing the second buffer
                                    // (RX_BUFFER_B then read second nibble of
                                    // the DB status reg)
      {
        statusDB >>= 4;
      }
      // setting the relevant bits in the main status register according to DB
      // status register
    ...
    if ((status & SYS_STATUS_RXFR_BIT_MASK) &&
        ((pdw3000local->stsconfig & DWT_STS_MODE_ND) == DWT_STS_MODE_ND)) {
      ...
      // here.
      switch (
          pdw3000local->dblbuffon) // check if in double buffer mode and if so
                                   // which buffer host is currently accessing
      {
      case DBL_BUFF_ACCESS_BUFFER_1: // accessing frame info relating to the
                                     // second buffer (RX_BUFFER_1)
        dwt_write8bitoffsetreg(
            RDB_STATUS_ID, 0,
            RDB_STATUS_CLEAR_BUFF1_EVENTS); // clear DB status register bits
                                            // corresponding to RX_BUFFER_1
        finfo16 = dwt_read16bitoffsetreg(INDIRECT_POINTER_B_ID, 0);
        break;
      case DBL_BUFF_ACCESS_BUFFER_0: // accessing frame info relating to the
                                     // first buffer (RX_BUFFER_0)
        dwt_write8bitoffsetreg(
            RDB_STATUS_ID, 0,
            RDB_STATUS_CLEAR_BUFF0_EVENTS); // clear DB status register bits
                                            // corresponding to RX_BUFFER_0
        finfo16 = dwt_read16bitoffsetreg(BUF0_RX_FINFO, 0);
        break;
      default: // accessing frame info relating to the second buffer
               // (RX_BUFFER_0) (single buffer mode)
    ...
    dwt_write32bitreg(SYS_STATUS_ID,
                      cia_err |
                          SYS_STATUS_ALL_RX_GOOD); // Clear all status bits
                                                   // relating to good reception
    ...
    if (pdw3000local->dblbuffon) // check if in double buffer mode and if so
                                 // which buffer host is currently accessing
    {
      // Free up the current buffer - let the device know that it can receive
      // into this buffer again
      dwt_signal_rx_buff_free();
    }
  }

  // RXFCE&~DISFCE|RXPHE|RXFSL|ARFE|RXSTO|RXOVRR. Real errored frame received,
  // so ignore FCE if disabled Handle RX errors events
  if (fstat & FINT_STAT_RXERR_BIT_MASK) {
    // Clear RX error events before the callback - this lets the host renable
    // the receiver inside the callback
    dwt_write32bitoffsetreg(SYS_STATUS_ID, 0,
                            SYS_STATUS_ALL_RX_ERR); // Clear RX error event bits
  ...
  if (fstat & FINT_STAT_RXTO_BIT_MASK) {
    // Clear RX TO events before the callback - this lets the host renable the
    // receiver inside the callback
    dwt_write8bitoffsetreg(
        SYS_STATUS_ID, 2,
        (uint8_t)(SYS_STATUS_ALL_RX_TO >>
                  16)); // Clear RX timeout event bits (PTO, RFTO)
 ...
 * I/O lines this function enables GPIOs 2 and 3 which are connected to LED3 and
 * LED4 on EVB1000
 *
 * input parameters
...
static void dwt_force_clocks(int clocks) {
  ...
  if (clocks == FORCE_CLK_SYS_TX) {
    uint16_t regvalue0 =
        CLK_CTRL_TX_BUF_CLK_ON_BIT_MASK | CLK_CTRL_RX_BUF_CLK_ON_BIT_MASK;
  ...
  if (clocks == FORCE_CLK_AUTO) {
    ...
    dwt_write16bitoffsetreg(
        CLK_CTRL_ID, 0x0,
        (uint16_t)DWT_AUTO_CLKS); // we only need to restore the low 16 bits as
                                  // they are the only ones to change as a result
                                  // of  FORCE_CLK_SYS_TX
  }
 ...
 * timing of delayed sending and reception. The value is at a 8ns resolution.
 *
 * input parameters
 * @param reftime - the reference time (which together with DX_TIME or TX
 * timestamp or RX timestamp time is used to define a transmission time or
 * delayed RX on time)
 *
 * output parameters none
 *
...
void dwt_setreferencetrxtime(uint32_t reftime) {
  dwt_write32bitoffsetreg(DREF_TIME_ID, 0,
                          reftime); // Note: bit 0 of this register is ignored
} // end dwt_setreferencetrxtime()

/*!
 * ------------------------------------------------------------------------------------------------------------------
 * @brief This API function configures the delayed transmit time or the delayed
 * RX on time The value is at a 8ns resolution.
 *
 * input parameters
 * @param starttime - the TX/RX start time (the 32 bits should be the high 32
 * bits of the system time at which to send the message, or at which to turn on
 * the receiver)
 *
 * output parameters none
 *
...
void dwt_setdelayedtrxtime(uint32_t starttime) {
  dwt_write32bitoffsetreg(DX_TIME_ID, 0,
                          starttime); // Note: bit 0 of this register is ignored
 ...
 * expected) if mode = DWT_START_TX_DELAYED - delayed TX (no response expected)
 * at specified time (time in DX_TIME register) if mode = DWT_START_TX_DLY_REF -
 * delayed TX (no response expected)  at specified time (time in DREF_TIME
 * register + any time in DX_TIME register) if mode = DWT_START_TX_DLY_RS  -
 * delayed TX (no response expected)  at specified time (time in RX_TIME_0
 * register + any time in DX_TIME register) if mode = DWT_START_TX_DLY_TS  -
 * delayed TX (no response expected)  at specified time (time in TX_TIME_LO
 * register + any time in DX_TIME register) if mode = DWT_START_TX_IMMEDIATE |
 * DWT_RESPONSE_EXPECTED - immediate TX (response expected - so the receiver
 * will be automatically turned on after TX is done) if mode =
 * DWT_START_TX_DELAYED/DLY_* | DWT_RESPONSE_EXPECTED - delayed TX (response
 * expected - so the receiver will be automatically turned on after TX is done)
 *               if mode = DWT_START_TX_CCA - Send the frame if no preamble
 * detected within PTO time if mode = DWT_START_TX_CCA  | DWT_RESPONSE_EXPECTED
 * - Send the frame if no preamble detected within PTO time and then enable RX*
 * output parameters
 *
...
int dwt_starttx(uint8_t mode) {
  ...
  if ((mode & DWT_START_TX_DELAYED) || (mode & DWT_START_TX_DLY_REF) ||
    ...
    checkTxOK = dwt_read8bitoffsetreg(
        SYS_STATUS_ID, 3); // Read at offset 3 to get the upper 2 bytes out of 5
    if ((checkTxOK & (SYS_STATUS_HPDWARN_BIT_MASK >> 24)) ==
        0) // Transmit Delayed Send set over Half a Period away.
    {
      ...
      // optionally could return error, and still send the frame at indicated
      // time then if the application want to cancel the sending this can be done
...
void dwt_forcetrxoff(void) {
  ...
  // Need to beware of interrupts occurring in the middle of following command
  // cycle We can disable the radio, but before the status is cleared an
  // interrupt can be set (e.g. the event has just happened before the radio was
  // disabled) thus we need to disable interrupt during this operation
  stat = decamutexon();
 ...
 * and off instead of being on all the time. The time spent in each state
 * (on/off) is specified through the parameters below. See DW3000 User Manual
 * section 4.5 "Low-Power SNIFF mode" for more details.
 *
 * input parameters:
 ...
 * size. The counter automatically adds 1 PAC size to the value set. Min value
 * that can be set is 1 (i.e. an ON time of 2 PAC size), max value is 15.
 * @param timeOff - duration of receiver OFF phase, expressed in multiples of
 * 128/125 탎 (~1 탎). Max value is 255.
 *
 * output parameters
 *
 * no return value
 ...
 * DWT_IDLE_ON_DLY_ERR         0x02    If delayed RX failed due to "late" error
 then if this flag is set the RX will not be re-enabled immediately, and device
...
int dwt_rxenable(int mode) {
  ...
  } else // delayed RX
  {
    ...
    temp1 = dwt_read8bitoffsetreg(
        SYS_STATUS_ID,
        3); // Read 1 byte at offset 3 to get the 4th byte out of 5
    if ((temp1 & (SYS_STATUS_HPDWARN_BIT_MASK >> 24)) !=
        0) // if delay has passed do immediate RX on unless DWT_IDLE_ON_DLY_ERR
           // is true
    {
      dwt_writefastCMD(CMD_TXRXOFF);

      if ((mode & DWT_IDLE_ON_DLY_ERR) ==
          0) // if DWT_IDLE_ON_DLY_ERR not set then re-enable receiver
      {
        dwt_writefastCMD(CMD_RX);
      }
 ...
 * units If set to 0 the timeout is disabled.
 *
 * output parameters
 *
 * no return value
...
void dwt_setrxtimeout(uint32_t time) {
  if (time > 0) {
    ...
    dwt_or16bitoffsetreg(SYS_CFG_ID, 0,
                         SYS_CFG_RXWTOE_BIT_MASK); // set the RX FWTO bit
  } else {
    dwt_and16bitoffsetreg(
        SYS_CFG_ID, 0,
 ...
 * size. The counter automatically adds 1 PAC size to the value set. Min value
 * that can be set is 1 (i.e. a timeout of 2 PAC size).
 *
 * output parameters
 *
 * no return value
 ...
 * bitmask are forced to selected state - i.e. the mask is written to the
 * register directly.
 *                   - otherwise (if set to DWT_DISABLE_INT) clear the
 * interrupts as selected in the bitmask output parameters
 *
 * no return value
 */
...
void dwt_readeventcounters(dwt_deviceentcnts_t *counters) {
  ...
  temp = dwt_read32bitoffsetreg(EVC_COUNT0_ID,
                                0); // Read sync loss (27-16), PHE (11-0)
  counters->PHE = temp & 0xFFF;
  counters->RSL = (temp >> 16) & 0xFFF;

  temp = dwt_read32bitoffsetreg(EVC_COUNT1_ID,
                                0); // Read CRC bad (27-16), CRC good (11-0)
  counters->CRCG = temp & 0xFFF;
  counters->CRCB = (temp >> 16) & 0xFFF;

  temp = dwt_read32bitoffsetreg(EVC_COUNT2_ID,
                                0); // Overruns (23-16), address errors (7-0)
  counters->ARFE = (uint8_t)temp;
  counters->OVER = (uint8_t)(temp >> 16);

  temp = dwt_read32bitoffsetreg(EVC_COUNT3_ID,
                                0); // Read PTO (27-16), SFDTO (11-0)
  counters->PTO = (temp >> 16) & 0xFFF;
  counters->SFDTO = temp & 0xFFF;

  temp = dwt_read32bitoffsetreg(EVC_COUNT4_ID,
                                0); // Read TXFRAME (27-16), RX TO (7-0)
  counters->TXF = (temp >> 16) & 0xFFF;
  counters->RTO = (uint8_t)temp;

  temp = dwt_read32bitoffsetreg(EVC_COUNT5_ID,
                                0); // Read half period warning (7-0) events
  counters->HPW = (uint8_t)temp;
  counters->CRCE =
      (uint8_t)(temp >> 16); // SPI CRC errors (23-16) warning events

  temp = dwt_read32bitoffsetreg(EVC_COUNT6_ID,
                                0); // Preamble rejections (11-0) events
...
void dwt_softreset(void) {
  // clear any AON configurations (this will leave the device at FOSC/4, thus we
  // need low SPI rate)
  dwt_clearaonconfig();
  ...
  // need to make sure clock is not PLL as the PLL will be switched off as part
  // of reset
  dwt_or8bitoffsetreg(CLK_CTRL_ID, 0, FORCE_SYSCLK_FOSC);
  ...
  // DW3000 needs a 10us sleep to let clk PLL lock after reset - the PLL will
  // automatically lock after the reset Could also have polled the PLL lock
  // flag, but then the SPI needs to be <= 7MHz !! So a simple delay is easier
  deca_sleep(1);

  // reset buffer to process RX_BUFFER_0 next - if in double buffer mode (clear
  // bit 1 if set)
...
/*!
 * ------------------------------------------------------------------------------------------------------------------
 * @brief This function returns the current value of XTAL trim that has been
 * applied. Following dwt_initialise this will be either the value read in OTP
 * memory or a default value.
 *
 *
 * input parameters
 *
 ...
 */
uint8_t dwt_getxtaltrim(void) { return pdw3000local->init_xtrim; }

/*!
 * ------------------------------------------------------------------------------------------------------------------
 * @brief This function will disable TX LDOs and allow TX blocks to be manually
 * turned off by dwt_disable_rftx_blocks
 *
 * input parameters
 * @param[in] switch_config - specifies whether the switch needs to be restored
 * to auto.
 *
 * output parameters
 * None
...
/*!
 * ------------------------------------------------------------------------------------------------------------------
 * @brief This function will enable TX LDOs and allow TX blocks to be manually
 * turned on by dwt_enable_rftx_blocks for a given channel
 *
 * input parameters
 * @param[in] channel - specifies the operating channel (e.g. 5 or 9)
 * @param[in] switch_config - specifies whether the switch needs to be
 * configured for TX
 *
 * output parameters
 *
...
static void dwt_enable_rf_tx(uint32_t channel, uint8_t switch_control) {
  ...
  dwt_or32bitoffsetreg(
      LDO_CTRL_ID, 0,
      (LDO_CTRL_LDO_VDDTX2_VREF_BIT_MASK | LDO_CTRL_LDO_VDDTX1_VREF_BIT_MASK |
       LDO_CTRL_LDO_VDDTX2_EN_BIT_MASK | LDO_CTRL_LDO_VDDTX1_EN_BIT_MASK));

  // Enable RF blocks for TX (configure RF_ENABLE_ID reg)
  if (channel == SEL_CHANNEL5) {
    dwt_or32bitoffsetreg(RF_ENABLE_ID, 0,
                         (RF_ENABLE_TX_SW_EN_BIT_MASK |
                          RF_ENABLE_TX_CH5_BIT_MASK | RF_ENABLE_TX_EN_BIT_MASK |
                          RF_ENABLE_TX_EN_BUF_BIT_MASK |
                          RF_ENABLE_TX_BIAS_EN_BIT_MASK));
  } else {
    dwt_or32bitoffsetreg(
        RF_ENABLE_ID, 0,
        (RF_ENABLE_TX_SW_EN_BIT_MASK | RF_ENABLE_TX_EN_BIT_MASK |
 ...
 * be repeated. If the value is less than the frame duration, the frames are
 * sent back-to-back.
 *
 * output parameters:
 * None
...
/*!
 * ------------------------------------------------------------------------------------------------------------------
 * @brief This function disables the automatic sequencing of the tx-blocks for a
 * specific channel.
 *
 * input parameters:
...
/*!
 * ------------------------------------------------------------------------------------------------------------------
 * @brief This function enables the automatic sequencing of the tx-blocks for a
 * specific channel.
 *
 * input parameters:
 * None
...
/*!
 * ------------------------------------------------------------------------------------------------------------------
 * @brief this function sets the DW3000 to transmit cw signal at specific
 * channel frequency
 *
 * input parameters:
 * @param channel - specifies the operating channel (e.g. 5 or 9)
...
/*!
 * ------------------------------------------------------------------------------------------------------------------
 * @brief this function sets the DW3000 to continuous tx frame mode for
 * regulatory approvals testing.
 *
 * input parameters:
 * @param framerepetitionrate - This is a 32-bit value that is used to set the
 * interval between transmissions. The minimum value is 4. The units are
 * approximately 8 ns. (or more precisely 512/(499.2e6*128) seconds)).
 * @param channel - specifies the operating channel (e.g. 5 or 9)
 *
 * output parameters
 *
...
void dwt_configcontinuousframemode(uint32_t framerepetitionrate,
                                   uint8_t channel) {
  // NOTE: dwt_configure and dwt_configuretxrf must be called before a call to
  // this API
  dwt_enable_rf_tx(channel, 1);
  dwt_enable_rftx_blocks(channel);
  dwt_force_clocks(FORCE_CLK_SYS_TX);
 ...
 * the DW IC. The values read here will be the current values sampled by DW IC
 * AtoD converters.
 *
 *
 * input parameters:
...
uint16_t dwt_readtempvbat(void) {
  ...
  /* Enable MS2 LDO - this is needed to read SAR if device is in IDLE_RC state,
   * this LDO is on in IDLE_PLL state */
  ldo_ctrl = dwt_read8bitoffsetreg(LDO_CTRL_ID, 0);
  dwt_or8bitoffsetreg(LDO_CTRL_ID, 0, LDO_CTRL_LDO_VDDMS2_EN_BIT_MASK);

  /*
   * SAR can read Vbat correctly by default. However, in order to read the
   * Tsense value, the appropriate bit must be set in the SAR_TEST register.
   */
  ...
  while (
      !(dwt_read8bitoffsetreg(SAR_STATUS_ID, SAR_STATUS_SAR_DONE_BIT_OFFSET) &
        SAR_STATUS_SAR_DONE_BIT_MASK))
    ;
 ...
 * conversion factor to give true temperature. The dwt_initialise needs to be
 * called before call to this to ensure pdw3000local->tempP contains the
 * SAR_LTEMP value from OTP.
 *
 * input parameters:
...
float dwt_convertrawtemperature(uint8_t raw_temp) {
  ...
  // the User Manual formula is: Temperature (캜) = ( (SAR_LTEMP �
  // OTP_READ(Vtemp @ 20캜) ) x 1.05)        // Vtemp @ 20캜
 ...
 * factor to give true voltage. The dwt_initialise needs to be called before
 * call to this to ensure pdw3000local->vBatP contains the SAR_LVBAT value from
 * OTP
 *
 * input parameters:
...
float dwt_convertrawvoltage(uint8_t raw_voltage) {
  ...
  // Bench measurements gives approximately: VDDBAT = sar_read * Vref / max_code
  // * 16x_atten   - assume Vref @ 3.0V
  realvolt =
      (float)((float)(raw_voltage - pdw3000local->vBatP) * 0.4f * 16 / 255) +
      3.0f;
 ...
 * bitfield setting) of the DW3000. The adjustment is a result of DW3000
 * internal PG cal routine, given a target count value it will try to find the
 * PG delay which gives the closest count value. Manual sequencing of TX blocks
 * and TX clocks need to be enabled for either channel 5 or 9. This function
 * presumes that the PLL is already in the IDLE state. Please configure the PLL
 * to IDLE state before calling this function, by calling dwt_configure.
 *
 * input parameters:
 * @param target_count - uint16_t - the PG count target to reach in order to
 * correct the bandwidth
 * @param channel - int - The channel to configure for the corrected bandwidth
 * (5 or 9)
 *
 * output parameters:
...
uint8_t dwt_calcbandwidthadj(uint16_t target_count, int channel) {
  ...
  // Write to the PG target before kicking off PG auto-cal with given target
  // value
 ...
 * register (PGC_STATUS) for a given PG_DELAY This is used to take a reference
 * measurement, and the value recorded as the reference is used to adjust the
 * bandwidth of the device when the temperature changes. This function presumes
 * that the PLL is already in the IDLE state.
 *
 * input parameters:
 * @param pgdly - uint8_t - the PG_DELAY (max value 63) to set (to control
 * bandwidth), and to find the corresponding count value for
 * @param channel - int - The channel to configure for the corrected bandwith (5
 * or 9)
 *
 * output parameters:
 * returns (uint16_t) - The count value calculated from the provided PG_DELAY
 * value (from PGC_STATUS) - used as reference for later bandwidth adjustments
 */
uint16_t dwt_calcpgcount(uint8_t pgdly, int channel) {
  ...
  count = dwt_read16bitoffsetreg(PGC_STATUS_ID,
                                 PGC_STATUS_PG_DELAY_COUNT_BIT_OFFSET) &
          PGC_STATUS_PG_DELAY_COUNT_BIT_MASK;
...
/*!
 * ------------------------------------------------------------------------------------------------------------------
 * @brief   This gets mic size in bytes and convert it to value to write in
 * AES_CFG
 ...
 * register Note, key register supports only 128-bit keys.
 *
 * output parameters
 *
 * no return value
 ...
 *          This is not a safe function as it is waiting for
 * AES_STS_AES_DONE_BIT_MASK forever.
 *
...
static uint8_t dwt_wait_aes_poll(void) {
  ...
  dwt_write8bitoffsetreg(
      AES_STS_ID, 0,
      tmp); // clear all bits which were set as a result of AES operation
 ...
 * DW3000 IC, when operating in CCM* AES mode expects the nonce to be programmed
 * into 4 words as follows: AES_IV0_ID[0] = nonce[10] AES_IV0_ID[1] = nonce[9]
 ...
 *
 */
static void dwt_update_nonce_CCM(uint8_t *nonce, uint16_t payload) {
  uint8_t iv[16];
  iv[0] = nonce[10];
  iv[1] = nonce[9];
  iv[2] = nonce[8];
 ...
 *
 * DW3000 IC, when operating in GCM AES mode expects the nonce to be programmed
 * into 3 words as follows: LSB (of nonce array) sent first. Nonce array is made
 * up of 12 bytes.
 *
 * @param nonce - Pointer to the nonce value to set
 *
 */
static void dwt_update_nonce_GCM(uint8_t *nonce) {
  dwt_writetodevice(AES_IV0_ID, 0, 12, nonce);
}

/*!
 * ------------------------------------------------------------------------------------------------------------------
 * @brief   This function provides the API for the job of encrypt/decrypt the
 * data block
 *
...
int8_t dwt_do_aes(dwt_aes_job_t *job, dwt_aes_core_type_e core_type) {
  ...
  if (job->mode == AES_Encrypt) {
    dwt_writetodevice(dest_reg, 0, job->header_len,
                      job->header); /*!< non-encrypted header */
  ...
  if ((job->dst_port == AES_Dst_Rx_buf_0) ||
    ...
    if (job->payload_len >
        STS_LEN_128BIT) // when writing to STS registers (destination port)
  ...
  /* fill header length and payload length - the only known information of the
   * frame. Note, the payload length does not include MIC and FCS lengths. So,
   * if overall rx_frame length is 100 and header length is 10, MIC is 16 and
   * FCS is 2, then payload length is 100-10-16-2 = 72 bytes.
   * */
  tmp = (DMA_CFG1_HDR_SIZE_BIT_MASK &
         (((uint32_t)job->header_len) << DMA_CFG1_HDR_SIZE_BIT_OFFSET)) |
        (DMA_CFG1_PYLD_SIZE_BIT_MASK &
         (((uint32_t)job->payload_len) << DMA_CFG1_PYLD_SIZE_BIT_OFFSET));
 ...
 * Low-Energy device (LE) address. For frame pending to function when the
 * correct bits are set in the frame filtering configuration via the
 ...
 * or IEEE 4z -8 (binary) The dwt_configure should be called prior to this to
 * configure other parameters
 *
 * input parameters
...
void dwt_configuresfdtype(uint8_t sfdType) {
  dwt_modify32bitoffsetreg(
      CHAN_CTRL_ID, 0, (uint32_t)(~CHAN_CTRL_SFD_TYPE_BIT_MASK),
